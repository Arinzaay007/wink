 ...
 * Guests get a one-click demo wallet; the whole flow settles on-chain.
 */
...
export default function TipForm({
  ...
  const ensureWallet = async (): Promise<DemoWallet | null> => {
    if (wallet) return wallet;
    const w = createDemoWallet();
    setWallet(w);
    // link to account if signed in (no-op for guests)
  ...
  const fundWallet = async (w: DemoWallet) => {
    setStage("funding");
    ...
    await new Promise((r) => setTimeout(r, 1500)); // let the faucet land
  ...
  const wink = async () => {
    ...
    try {
      ...
      if (!w) throw new Error("could not create wallet");
      ...
      // top up if needed
      const bal = balance ?? (await fetchBalance(w.address));
      if (bal * 1_000_000 < amountMicro + 100_000) await fundWallet(w);
      ...
      // Tempo finality is ~500ms; poll our verification endpoint
      setStage("confirming");
      let confirmed = false;
      ...
      if (!confirmed) throw new Error("broadcast but not verified yet — check your dashboard");
      setStage("done");
      setBalance(await fetchBalance(w.address));
    } catch (e) {
  ...
  return (
    <div className="card p-6">
      ...
      <button
        ...
        onClick={wink}
        disabled={["signing", "confirming", "funding"].includes(stage)}
      >
        {stage === "funding" && "Funding demo wallet…"}
        {stage === "signing" && "Sending on-chain…"}
